...
K_opt_range = [1 5 10 25 50 100:50:K_opt_max];
...
v_true = zeros(n_x, H, K_test);
e_true = zeros(n_y, H, K_test);

for n = 1:K_test
    for t = 1:H
        v_true(:,t, n) = mvnrnd(zeros(n_x, 1), Q_true)';
        e_true(:,t, n) = mvnrnd(zeros(n_y, 1), R_true);
    end
end
...
for K_opt = K_opt_range
    ...
    for k = 1:K_test
        X_tmp(:,1, k) = x_training(:, end);

        for t = 1:H
            X_tmp(:, t+1, k) = f_true(X_tmp(:, t, k), U_scenario(:, t)) + v_true(:,t,k);
            Y_tmp(:, t, k) = g_true(X_tmp(:, t, k), U_scenario(:, t)) + e_true(:,t,k);
        end
    end
    ...
    Accuracy_scenario(cntk) = 100 * sum(reshape(C, K_test, 1)) / K_test;
    ...
    for alpha = Alpha_range
        ...
        for k = 1:K_test
            ...
            for t = 1:H
                X_tmp(:, t+1, k) = f_true(X_tmp(:, t, k), U_kernel(:, t)) + v_true(:,t,k);
                Y_tmp(:, t, k) = g_true(X_tmp(:, t, k), U_kernel(:, t)) + e_true(:,t,k);
            end
        end
        ...
        Accuracy_kernel(cntk,cnta) = 100 * sum(reshape(C, K_test, 1)) / K_test ;
        cnta = cnta + 1;
    end
    cntk = cntk + 1;
