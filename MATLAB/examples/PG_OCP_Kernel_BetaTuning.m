...
B_range = zeros(B_cnt, 2, iter_max * 2 + 1);

B_range(:,:,1) = repmat(B, B_cnt, 1);

K_opt = 100;

K_test = K - K_opt;

Accuracy_kernel = zeros(2, B_cnt, iter_max);
...
for iter = 1:iter_max
    iter
    for ts = 1
        B_range(:,ts,cnts) = linspace(0.75* B_range(1,ts,cnts), 1.25* B_range(1,ts,cnts), B_cnt)';
        if ts == 1
            B_range(B_range(:,ts,cnts) >= 1,ts,cnts)= 1;
        end
        for B_iter = 1:B_cnt            
            [U_kernel, X_kernel, Y_kernel] = Solve_OCP_Kernel_maxConstraint_betainput(PG_samples, x_vec_0, v_vec, e_vec, H, K_opt, phi, g, n_x, n_y, n_u, y_min, y_max, alpha, sigma, B_range(B_iter,1,cnts), B_range(B_iter,2,cnts));
            ...
            Accuracy_kernel(ts,B_iter,iter) = 100 * sum(reshape(C, K_test, 1)) / (K - K_opt);
        end
        [~, B_opt_idx] = min((Accuracy_kernel(ts, :, iter) - 1 + alpha).^2);

        minIdx = find(Accuracy_kernel(ts, :, iter) == Accuracy_kernel(ts, B_opt_idx, iter));

        [~, minIdx_idx] = min((minIdx - ceil(B_cnt/2)).^2);

        B_opt_idx = minIdx(minIdx_idx);

        B_opt_idx

        B_range(:,:,cnts+1) = repmat(B_range(B_opt_idx, :, cnts), B_cnt, 1);

        B = [B; B_range(B_opt_idx, :, cnts)];
